...
export default async function GroupedProductsListPage({ searchParams }: PageProps) {
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <ProductGroupList expanded={expanded} limit={limit} groupBy={groupBy} />
    </HydrationBoundary>
  );
}
```

## Client (product-group-list.tsx)
...
export function ProductGroupList({ expanded, limit, groupBy }: ProductGroupListProps) {
  const trpc = useTRPC();
